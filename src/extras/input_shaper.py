...
class AxisInputShaper:
    ...
    def set_shaper_kinematics(self, sk):
        success = lib.input_shaper_set_shaper_params(
            sk, self.axis.encode(), self.n, self.A, self.T) == 0
        if not success:
            self.disable_shaping()
            lib.input_shaper_set_shaper_params(
...
class InputShaper:
    ...
    def _get_input_shaper_stepper_kinematics(self, stepper):
        ...
        is_sk = ffi.gc(lib.input_shaper_alloc(), lib.free)
        stepper.set_stepper_kinematics(is_sk)
        res = lib.input_shaper_set_sk(is_sk, sk)
        if res < 0:
    ...
    def _update_input_shaping(self, error=None):
        ...
        for s in kin.get_steppers():
            ...
            if is_sk is None:
                continue
            old_delay = lib.input_shaper_get_step_generation_window(is_sk)
            for shaper in self.shapers:
                if shaper in failed_shapers:
                    continue
                if not shaper.set_shaper_kinematics(is_sk):
                    failed_shapers.append(shaper)
            new_delay = lib.input_shaper_get_step_generation_window(is_sk)
